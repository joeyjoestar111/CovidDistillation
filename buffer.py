...
def main(args):
    ...
    for it in range(0, args.num_experts):
        ...
        wandb.init(sync_tensorboard=False,
                entity='21307140005',
                project="mtt-buffer50-{}-{}-lr{}-mom{}-ep{}".format(args.dataset, args.model, args.lr_teacher, args.mom ,args.train_epochs),
                name='resnet18-{}'.format(it)
            #    name='test'
               )
        ...
        for e in range(args.train_epochs):
            ...
            test_loss, test_acc, test_auc = epoch("test", dataloader=testloader, net=teacher_net, optimizer=False,
                                         criterion=criterion, args=args, aug=False)
            if args.dataset=='PG':
                print("Itr: {}\tEpoch: {}\tTrain Acc: {}\tTrain Bag Acc: {}\tTest Acc: {}\tTest Bag Acc: {}".format(it, e, train_acc, train_auc, test_acc, test_auc))
            ...
            if args.dataset=='PG':
                ...
                wandb.log({'Test Bag Acc': test_auc}, step=e)
            elif args.dataset=='COVID':
                wandb.log({'Train Bag Acc': train_auc}, step=e)
                wandb.log({'Test Bag Acc': test_auc}, step=e)
            else:
