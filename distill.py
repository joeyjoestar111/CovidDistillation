...
def main(args):
    ...
    wandb.init(sync_tensorboard=False,
                project="Covid Distillation",
                entity="21307140005",
                name='Covid-'+args.pix_init+'-ipc_{}-max_start_epoch_{}-expert_epochs_{}-syn_steps_{}-lr_teacher_{}-lr_lr_{}-lr_img_{}'.format(args.ipc, args.max_start_epoch, args.expert_epochs, args.syn_steps, args.lr_teacher, args.lr_lr, args.lr_img),
                config=args,
            )
    ...
    for it in range(0, args.Iteration+1):
        ...
        if it in eval_it_pool:
            for model_eval in model_eval_pool:
                ...
                for it_eval in range(args.num_eval):
                    net_eval = get_network(model_eval, channel, num_classes, im_size, no_frz=args.no_frz).to(args.device) # get a random model
                ...
                wandb.log({'Bag_Acc/{}'.format(model_eval): auc_test_mean}, step=it)
                wandb.log({'Max_Bag_Acc/{}'.format(model_eval): best_acc[model_eval]}, step=it)
                wandb.log({'Std_Bag_Acc/{}'.format(model_eval): auc_test_std}, step=it)
                wandb.log({'Max_Std_Bag_Acc/{}'.format(model_eval): best_std[model_eval]}, step=it)
        ...
        if (save_this_it or it % 1000 == 0):
            with torch.no_grad():
                ...
                torch.save(syn_lr.item(), os.path.join(save_dir, "lr_{}.pt".format(it)))
        ...
        student_net = get_network(args.model, channel, num_classes, im_size, dist=False, no_frz=args.no_frz).to(args.device)  # get a random model
        ...
        for step in range(args.syn_steps):
            ...
            x = student_net(x=x, flat_param=forward_params)
