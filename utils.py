...
class PG(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_path = result['img_info']['filename']
        label = result['gt_label']
        bagname = result['bag']
        img = Image.open(img_path).convert('RGB')
...
def get_dataset(dataset, data_path, batch_size=1, subset="imagenette", args=None):
    ...
    if dataset == 'CIFAR10':
        ...
        class_names = ['MSI', 'MSS']
        # CRC_train = '/home/sjtu/scratch/lfzhou/CRC/CRC_DX_train'
        # CRC_test = '/home/sjtu/scratch/lfzhou/CRC/CRC_DX_test'
        # train_ann_path = '/home/sjtu/scratch/lfzhou/CRC/annotation/train_ann.txt'
        # test_ann_path = '/home/sjtu/scratch/lfzhou/CRC/annotation/test_ann.txt'
        ...
        task = 0
        class_names = ['0','1','2']
        PG_train = '/shared/dqwang/scratch/yunkunzhang/patchGastricADC22/patches_captions'
        PG_test = '/shared/dqwang/scratch/yunkunzhang/patchGastricADC22/patches_captions'
        train_ann_path = '/shared/dqwang/scratch/zltan/data_anno/subtype_cls3_ann/train_all_0.2.txt'
        test_ann_path = '/shared/dqwang/scratch/zltan/data_anno/subtype_cls3_ann/val_0.8.txt'
        dst_train = PG(PG_train, train_ann_path, cls_ind=task, train=True, transform=transforms.Compose(train_list)) # no augmentation
        dst_test = PG(PG_test, test_ann_path, cls_ind=task, train=False, transform=transforms.Compose(test_list))
        # class_names = dst_train.classes
        class_map = {x: x for x in range(num_classes)}
        ...
        task = 0
        class_names = ['0','1','2','3']
        COVID_train = '/shared/dqwang/scratch/zltan/COVID-19_Radiography_Dataset'
        COVID_test = '/shared/dqwang/scratch/zltan/COVID-19_Radiography_Dataset'
        train_ann_path = '/shared/dqwang/scratch/zltan/COVID-19_Radiography_Dataset/train_0.8.txt'
        test_ann_path = '/shared/dqwang/scratch/zltan/COVID-19_Radiography_Dataset/test_0.2.txt'
        dst_train = PG(COVID_train, train_ann_path, cls_ind=task, train=True, transform=transforms.Compose(train_list)) # no augmentation
        dst_test = PG(COVID_test, test_ann_path, cls_ind=task, train=False, transform=transforms.Compose(test_list))
        # class_names = dst_train.classes
        class_map = {x: x for x in range(num_classes)}
    ...
    testloader = torch.utils.data.DataLoader(dst_test, batch_size=16, shuffle=False, num_workers=0)
...
def get_network(model, channel, num_classes, im_size=(32, 32), dist=True, no_frz=False):
    ...
    if model == 'MLP':
        ...
        # net = MyResNet18(ck_path='/home/sjtu/scratch/lfzhou/mtt-distillation/ckpt/r18_imgpre.pth')
...
def epoch(mode, dataloader, net, optimizer, criterion, args, aug, texture=False):
        ...
        if mode == 'test' or mode == 'train':
            for j in range(len(bagname)):
                if bagname[j] not in bag:
                    ...
                    if args.dataset=='PG':
                        bag_score[bagname[j]]=torch.zeros(3).cuda()
                    elif args.dataset=='COVID':
                        bag_score[bagname[j]]=torch.zeros(4).cuda()
                    else:
                        bag_score[bagname[j]] = 0
                if args.dataset=='PG'or args.dataset=='COVID':
                    bag_score[bagname[j]]+=output[j]
                else:
    ...
    if mode == 'test' or mode == 'train':
        if args.dataset=='PG'or args.dataset=='COVID':
            corrects=0
            if args.dataset=='PG':
                bag_labels = [[1 if i==kv[1] else 0 for i in range(3)] for kv in sorted(bag_label.items(), key=lambda x: x[0])]
            elif args.dataset=='COVID':
                bag_labels = [[1 if i==kv[1] else 0 for i in range(4)] for kv in sorted(bag_label.items(), key=lambda x: x[0])]
            for b in bag_score.keys():
                mean = bag_score[b] / bag[b]
                bag_pred_dict[b]=mean
                # print(mean)
                pred = mean.argmax()
                # print(mean,pred)
                if pred == bag_label[b]:
                    corrects+=1
            bag_results = [kv[1].cpu() for kv in sorted(bag_pred_dict.items(), key=lambda x: x[0])]
            # print(bag_labels,bag_results)
            # auc = roc_auc_score(bag_labels,bag_results)
            nbag = len(bag_score.keys())
            bag_acc = corrects/nbag
            # auc = 0
            return loss_avg, acc_avg, bag_acc
